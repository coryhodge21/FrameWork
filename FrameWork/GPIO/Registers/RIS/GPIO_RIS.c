...
void RIS_set_this(RIS_e Mask){
    ...
    *(volatile unsigned int *)GPIO_RIS_BASE_ADDR |= (Mask);
}
...
int RIS_read_this(RIS_e Mask){
    ...
    return (*(volatile unsigned int *)GPIO_RIS_BASE_ADDR & (Mask));
}
...
void RIS_write_this(RIS_e Mask, int msg) {
    ...
    *(volatile unsigned int *)GPIO_RIS_BASE_ADDR = (Mask & msg);
}
//! \brief Constructor for this Register
//! \brief param[out] Register with assigned address and function pointers
GPIO_RIS_t init_GPIO_RIS(void){

	// Create Register
	GPIO_RIS_t	Register;

    // For Direct Memory Location Access
    Register.BASE_ADDR = GPIO_RIS_BASE_ADDR;
